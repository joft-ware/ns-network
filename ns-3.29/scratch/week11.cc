    ...
    //   printf("flow 11111 event\n");
      ...
      // printf("flow 22222 event\n");
        ...
        NS_LOG_UNCOND("2\t" << Simulator::Now().GetSeconds()
             << "\t" << bytes2 * 8 / 1000000 / (Simulator::Now().GetSeconds()-6) * 10 );
        std::cout << "2\t" << Simulator::Now().GetSeconds()
              << "\t" << bytes2 * 8 / 1000000 / (Simulator::Now().GetSeconds()-6) * 10 << std::endl;
    }
      ...
      // printf("flow background event\n");
        ...
        NS_LOG_UNCOND("0\t" << Simulator::Now().GetSeconds()
             << "\t" << bytes2 * 8 / 1000000 / (Simulator::Now().GetSeconds()-1) * 10 );
...
main (int argc, char *argv[])
{
  ...
  PacketSinkHelper sink_flow2("ns3::TcpSocketFactory", 
    Address(InetSocketAddress(Ipv4Address::GetAny(), port)));
  ApplicationContainer sinkApp_flow2 = sink_flow2.Install(nodes.Get(0));
  sinkApp_flow2.Start(Seconds(6.0));
